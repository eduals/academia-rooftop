...
(function() {
    ...
    var module = {
      ...
      applyMasks: function() {
        ...
        var valorEsperadoInput = document.getElementById('valor_esperado_cliente');
        if (valorEsperadoInput) {
          valorEsperadoInput.addEventListener('input', function(e) {
            self.maskCurrency(e.target);
          });
        }

        var valorAvaliacaoInput = document.getElementById('valor_avaliacao_cliente');
        if (valorAvaliacaoInput) {
          valorAvaliacaoInput.addEventListener('input', function(e) {
            self.maskCurrency(e.target);
          });
        }
      },
      ...
      collectFormData: function() {
        ...
        // Parse the new currency fields
        if (data.valor_esperado_cliente) {
          data.valor_esperado_cliente = this.parseCurrency(data.valor_esperado_cliente);
        }
        
        if (data.valor_avaliacao_cliente) {
          data.valor_avaliacao_cliente = this.parseCurrency(data.valor_avaliacao_cliente);
        }
